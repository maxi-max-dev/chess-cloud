...
**真机验收**（无头 Chrome + CDP，本地约 60 秒）：

```bash
node live-check.mjs
```

期望：`全绿：25/25 项通过`。它会自己起静态服务器、拉起无头 Chrome、走棋、截图、对数。
...
| 文件 | 行数 | 干什么 |
|---|---|---|
| `index.html` | 842 | 整个页面：SVG 棋盘、three.js 星云、分叉图、所有交互 |
...
| `live-check.mjs` | 375 | 无头 Chrome 真机验收（含零依赖 PNG 解码做像素统计） |
...
6. **AI 应手有硬上限 ≤3 秒。** 现在靠 `search()` 的迭代加深 + `AI_BUDGET_MS = 1500` 预算保证，
   时间到就交上一层搜完的最好走法。加任何耗时功能前，先想清楚会不会挤掉这 3 秒。
...
| e4 后 AI 应手（外部秒表） | 约 2.0–2.4 s，搜到 4 层 |
| `node verify.mjs` | 约 7 s，8/8 绿 |
| `node live-check.mjs` | 约 60 s，25/25 绿 |
...
3. **手机端适配**（BLOCKED #10）。现在面板全是固定像素宽，竖屏会挤成一团。

4. **升变选棋子的界面**（BLOCKED #7）。现在默认成后。
   `tryMove(from, to, promotion)` 已经收第三个参数，接个 UI 就行。

5. **搜索加和棋判定**（BLOCKED #4）。现在只判将死和困毙，不判三次重复 / 50 步 / 子力不足，
...
- 最新 commit：`6791bd3 主视图换成平面分叉图，星云降级成小窗`。
- 本地 25/25 绿，线上 25/25 绿，`node verify.mjs` 8/8 绿。
