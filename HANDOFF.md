...
截止 2026-08-01，项目已经定位为“一个棋局未来地图产品 + 两套独立真实棋核”：规则分开，
...
FEN 或丢失预演。两页都会先只播放用户选择的第一步，再完整列出对方全部合法回应；金色第一名只是
引擎建议。只有用户显式点中某个回应，主棋盘才播放第二拍并停在该条件局面。确认仍只提交第一步。
...
| `xiangqi-live-check.mjs` | 中国象棋 58 项真 Chrome / 路由 / 未知回应 / 上一步 / 威胁 / 棋子材质 / 手机裁判 |
...
点任一未来节点只建立蓝色用户首步预演，FEN 和实战路径不变；首拍结束后页面从该真实子局面生成
对方全部合法回应。金色只标浅层排序建议，蓝色第二拍必须来自用户明确点击。3D 感全景与 2D 推演树
必须保留同一个候选、回应集合和预演 FEN。只有“走这步”或棋盘合法落点才真正落子。排序会看对方
回应和一步合法回吃，但仍不是预测、胜率或概率。
...
- 第一次选择只生成一拍，phase 从 `playing` 进入 `await-reply`；此时 `frontier` 必须完整列出该局面
  全部合法回应，且只有一条 `suggested=true` 的分析建议，不能自动写进 `selectedPath`；
- 用户显式选回应后才进入 `conditional-playing → conditional-settled`；减少动态对应
  `conditional-static`。第一步已结束棋局时进入 `terminal`，不显示“从 0 种回应中选择”；
...
- 动画结束后停在最终推演局面供分析，主棋盘在整个预演期间只读；只有明确确认按钮能提交第一步。取消、
  reset 或实战变化必须重绘根局面；
- `prefers-reduced-motion: reduce` 下不播放位移：先静态停在首步，显式选回应后才投影两拍条件局面；
  播放中切换偏好也要立即收束，切回普通动态不能补播旧路线。
...
`xiangqi-live-check.mjs` 用 `EXPECTED_RESULTS = 58` 锁定 58 项，覆盖：
...
- 首拍必须只有一条静态路径并进入 `await-reply`；显式点回应后才出现第二条路径，终局零回应进入
  `terminal`。预演期间主盘完全只读，只能由“走这步”提交第一步。
...
- 修复后当前门户锁定 **25/25**，国际真机锁定 **101/101**，中国象棋真机锁定 **58/58**；
  裁判期望项数只增加，没有降低。
...
| `npm test` | 国际 8/8 + 中国 19/19 + 门户 25/25 |
| `node live-check.mjs` | 101/101 |
| `node xiangqi-live-check.mjs` | 58/58 |
...
| 两页主棋盘预演 | 首拍停在 `await-reply`；显式回应后才播放条件拍；终局零回应；实战不变；减少动态同语义 |
...
- 中国象棋保留独立规则 / Worker，3D 感全景与 2D 推演共用候选、全部合法回应、分析建议、走后
  FEN 和路线后果。
- 两页都先只播放用户明确选择的第一步；首拍稳定后列出全部真实回应，用户再点某一项才播放条件
  第二拍。金色建议不再冒充已知未来；动画使用独立 FEN 重放与 line ID 隔离，预演期只读。
...
- 当前源码基线：`npm test` 为 **8/8 + 19/19 + 25/25**，国际真机 **101/101**，
  中国真机 **58/58**。
