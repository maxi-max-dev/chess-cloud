...
2. `xiangqi.html`：中国象棋，人执红、AI 执黑。是真正的 9×10 规则核，不是给国际象棋换一套棋子。
   ...
   危险环与 attacker→target 箭头只是这些事实的可读视觉层。
...
- 威胁固定局面、真实攻击者 / 保护者 / SVG 箭头 / “危”字逐项同源；
- 威胁动效有限次停止，减少动态时仍保留静态环、攻击线和文字；
...
| 中国威胁 | 常驻危险环 + 真实 attacker→target 箭头 + 攻击者 / 保护者；仍只是几何攻击线 |
| 中国威胁动效 | 有限次后停止；减少动态时保留静态环、线、文字；持续 rAF 为 0 |
