...
后自动选择金色分析主线、连续云演 2 ply 并停住，但所有合法回应仍完整可选，换线会立刻接管。
...
推演树必须保留同一个候选、回应集合和预演 FEN。只有“走这步”或棋盘合法落点才真正落子。排序会
...
- 中国象棋自动主线从 `autoplay_started` 起连续 2 ply，并在 8 秒内进入稳定
...
- 动画结束后停在最终推演局面供分析，主棋盘在整个预演期间只读；只有明确确认按钮能提交第一步。取消、
  reset 或实战变化必须重绘根局面；
- `prefers-reduced-motion: reduce` 下不播放位移：中国象棋自动静态投影两拍，国际象棋保持显式条件
...
- 首次加载零点击自动选择主线、连续提交 2 ply、8 秒内稳定，实战 FEN / 历史不变；
- `positionKey / positionId / requestId / runId` 生命周期与动画回调双重身份校验；
- `motion_committed → landing_impact → threats_revealed` 两拍顺序；
- 暂停 / 继续冻结 run 和预算，reset 后旧 run 回调被拒；
- `prefers-reduced-motion` 直接得到同一 2 ply 静态结果且没有运动棋子。
...
| 主棋盘预演 | 国际显式选路；中国零点击 2 ply / 8 秒硬预算；实战不变；减少动态同结果 |
...
- 国际象棋先播放用户明确选择的第一步；中国象棋首次加载 / reset 自动选分析主线并连续云演
  2 ply。两页都完整保留真实回应，金色建议不冒充已知未来；动画使用独立 FEN 重放，预演期只读。
