...
照着做就能接着干。截止 2026-07-30，当前最终架构已经写进这份文档；固定验收现在是
`verify.mjs` 8 项、`live-check.mjs` 70 项，最终本地与线上输出见第 6、9 节。
...
1. **「这枚棋子怎么走、对手会怎么回」** —— 点任意白棋，助手列出这枚棋子的前三候选，
   先用黄线动画演示“你的一步”，再用蓝线演示“对手优先防的强回应”。它明确是引擎强度，
...
2. **「接下来到底有多少种可能」** —— 以当前局面为根，往下四层全展开成三维父子路径网
   ...
   节点恰有一条真实入边。左上小窗可点开全屏，带“现在”、SAN、局势文字和当前选路标签。
...
脚本用 `EXPECTED_RESULTS = 70` 锁定固定 70 项；成功时应输出 `全绿：70/70 项通过`。它会自己起
静态服务器、拉起无头 Chrome、走棋、截图、对数，
并切到 390×844 / 667×375 / 844×390 / 1024×768 验手机与平板布局、
真实触摸/滑动、黑白棋子像素、棋子助手、AI 竞态、背景和路径网全屏。
...
| 文件 | 行数 | 干什么 |
|---|---|---|
| `index.html` | 2920 | 整个页面：分层 SVG 3D 棋子、棋子助手、three.js 路径网、分叉、响应式与交互 |
...
- 每个候选的对手回应来自 `rankMoves(candidate.after, {depth: 1})`，黑方排序方向由 `rankMoves`
  自己处理。文案只能说“优先防的强回应（按引擎强度）”，不能说历史胜率或概率。
- 动画只重建 `boardOverlay`，绝不 `game.move()`。切换棋子、重开、真实落子都会换 `lineId`
  或清空旧动画。棋盘格支持触摸、鼠标和 Enter/Space。
...
- 每个未来节点恰有一条父→子 `THREE.LineSegments` 入边；零 `THREE.Points`。每批单独开一个
  「刚好这么大」的非索引 `BufferGeometry`，所以真实路径数是 `position.count / 2`。
  **不预分配、不用 drawRange。** `selected-route` 单独挂在 scene，不进入 `layers` 账本。
- 移动端路径网离屏时只保留真实第 0–3 层并释放第 4 层 geometry；回到视口时用保存的第 3 层 FEN
...
`live-check.mjs` 现在固定验这些（70 项，`EXPECTED_RESULTS = 70`，少跑一项也会失败）：

| 组 | 验什么 |
|---|---|
| ① | `__cloudStats().nodes` 与 `count(同 fen, 同 depth)` 逐层相等。起始局面 1/20/400/8902/197281，**AI 应手后的局中局面 1/30/654/20144/475842 也对**（这些数页面都没写死） |
| ② | 零 `THREE.Points`；根短边终点在原点；L1 边数等于合法走法；所有对象是真实非索引 `LineSegments`，顶点成对、颜色对齐、无隐藏/孤儿/drawRange；截图对撞正常图、隐藏全网、单独隐藏 L4；真实差分中暖冷两色可见 |
| ③ | 分叉图每列总数和卡片数对账；排序方向对；每列恰有一张选中卡、一条选中边和一段连续主干；真实点击改路；真实展开→收起；推荐区外的选中卡在收起后仍可见；走满两回合后仍对 |
| ④ | 390×844、667/844 横屏与 1024 平板；真实页面/分叉滑动；≥44px；背景首尾；真全屏四角；真实 SAN 标签和多步主路径；触摸标签只选路不落子；拖动后标签显隐；按钮收起；移动路径网 L3→L4→离屏释放回 L3→回屏完整重建 L4；真实 touch e4 后 AI ≤3 秒 |
...
| 移动路径网生命周期 | 屏外 L3 / 入屏 L4 / 离屏释放回 L3 / 回屏完整重建 L4 |
| 棋子 renderer | 六种分层 SVG 3D 视觉造型；零 Unicode；不是 WebGL mesh |
| 棋子助手 | 候选 depth=2；回应 depth=1；黄/蓝两段 SVG 动画；预演不改 FEN |
