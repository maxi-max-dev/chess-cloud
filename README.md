...
2 ply，国际象棋仍由用户显式选择条件路线；两者都能看清吃子、将军、威胁与暴露等直接后果。
...
中国象棋首次加载或重置后会自动呈现全部真实候选，选中浅层分析主线，并连续播放“红方候选 +
黑方一种可能回应”共 2 ply；从 `autoplay_started` 到稳定局面硬限 8 秒，通常约 2 秒完成。它不是
预测：金色仍只是一条分析建议，所有合法回应继续完整保留，用户随时可以暂停、继续、换线接管、退出
或重置。页面隐藏或棋盘离开视口时自动暂停，旧 run 的定时器和动画回调不能推进新局面。

国际象棋维持显式选路：先播放用户选择的第一步，停住并列出对方全部真实回应，用户再点某个回应才
播放第二拍。两页的预演都从当前 FEN 独立重放，不改实战记录；棋盘只读，只有“走这步”会真正提交
第一步。减少动态模式不播放位移，直接显示相同的条件结果。手机竖屏保留画中画棋盘与横滑回应条。
...
浅层排序第一名和它的分析回应，连续演完 2 ply 后停住；所有黑方合法回应仍完整列出，换线会立即
接管并使旧 run 失效。路线语境栏在“运动完成原子提交 → 落点脉冲 → 威胁展开”的顺序中更新；只有
“走这步”或棋盘合法落点才真正改变实战 FEN。节点分叉数来自真实子局面，排序不是预测、历史胜率
或概率。
...
| `xiangqi-verify.mjs` / `xiangqi-live-check.mjs` / `xiangqi-self-play.mjs` | 中国象棋棋核、63 项真 Chrome、固定 10 局稳定性验收 |
