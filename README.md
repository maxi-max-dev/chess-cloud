...
4 ply，国际象棋仍由用户显式选择条件路线；两者都能看清吃子、将军、威胁与暴露等直接后果。
...
黑方回应—红方接续—黑方回应”共 4 ply；从 `autoplay_started` 到稳定局面硬限 8 秒。它不是预测：
金色仍只是一条分析建议，所有合法回应继续完整保留，用户随时可以暂停、继续、换线接管、退出或重置。
棋盘下方始终显示“采纳首步继续 / 返回棋盘自己下”，避免推演遮住真正下棋。页面隐藏或棋盘离开
视口时自动暂停，旧 run 的定时器和动画回调不能推进新局面。

国际象棋维持显式选路：先播放用户选择的第一步，停住并列出对方全部真实回应，用户再点某个回应才
播放第二拍。两页的预演都从当前 FEN 独立重放，不改实战记录；棋盘只读，只有“采纳首步继续 / 走这步”会真正提交
...
浅层排序第一名和两组合法候选 / 回应，连续演完 4 ply 后停住；所有黑方合法回应仍完整列出，换线会立即
