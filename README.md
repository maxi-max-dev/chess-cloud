...
都用同一套视觉与交互语言，一眼查看当前全部真实下一步，再显式选择一种可能回应继续分析，并看清吃子、
将军、威胁与暴露等直接后果。
...
两种棋的未来地图都不再只是卡片或线图。点第一步时，主棋盘只播放这一步，然后停住并列出对方
全部真实合法回应；金色只是一条引擎建议，不代表真人或 AI 必然这样走。用户再点某个回应后，才播放
第二拍并停在该条件局面。预演从当前 FEN 独立重放，不改实战记录；棋盘只读，只有“走这步”会真正
提交第一步。快速换回应会取消旧动画并生成新路线；减少动态模式直接显示同一个条件结果。手机竖屏
会保留固定画中画棋盘，回应条可横滑查看。
...
第一层全部真实路线放进同一视角；两种模式共用同一条用户预演与同一个走后局面。点任一节点先只
预演你的一步，随后完整列出黑方全部合法回应；浅层排序第一名用金色标成“建议先检查”，只有用户
明确点中某个回应才继续条件预演。路线语境栏同步显示吃子、将军和新进入攻击线的棋子；只有“走这步”
或棋盘合法落点才真正改变 FEN。节点分叉数来自真实子局面，排序不是预测、历史胜率或概率。
...
| `verify.mjs` / `live-check.mjs` / `self-play.mjs` | 国际象棋棋核、101 项真 Chrome、10 局稳定性验收 |
| `xiangqi.html` / `xiangqi-engine.js` / `xiangqi-worker.js` | 中国象棋 UI、规则 / 唯一评估 / 搜索、搜索与分叉 Worker |
| `xiangqi-verify.mjs` / `xiangqi-live-check.mjs` / `xiangqi-self-play.mjs` | 中国象棋棋核、58 项真 Chrome、固定 10 局稳定性验收 |
| `portal-check.mjs` | 首页、共享未来地图契约、两个规则入口和运行代码红线的 25 项静态验收 |
...
# 快速、固定的棋核与路由裁判
npm test
# 国际象棋 8/8 + 中国象棋 19/19 + 统一门户 25/25
...
- `npm test`：国际象棋 **8/8**、中国象棋 **19/19**、统一门户 **25/25**；
